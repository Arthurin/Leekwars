/**
 *  j'ai pas commenté, désolé...
 * 	et c'est encore un peu brouillon ^^ 
 *  Pour tester : cf le block "test" (=> dangerCombo(getDanger(getAliveEnemies(),ME));)
 */
	...
	for (var ennemi in ennemis) {
		...
		for(var tool in tools) {
			...
			for(var dist=0; dist<=tailleAOE; dist++) {
				...
				ValueWeapons[tool][dist][ennemi]=(getValue(tool, INFO_LEEKS[ennemi], INFO_LEEKS[getLeek()], dist))[MOYEN];
			}
		}
	}
	return @ValueWeapons;
...
/*
 * Proposition de v2_bis pour prendre en compte l'aoe
 * */
function getDanger2(ennemis,leek) {
	var danger = [];
	var mesCells = getAccessibleCells(getCell(leek),getMP(leek));
	var casesEnnemis = getAccessibleCasesLeeks(ennemis);
	var valueWeapons = getValueWeapons(ennemis);
	for (var cell: var d in mesCells) {
		if (danger[cell]===null)  danger[cell] = [];
		for(var weapon :var distAOE in valueWeapons) {
		//distAOE, pour chaque distance d'AOE : [ennemyId:maxDmgArme]
		// 43 : [[1 : 245.0], [1 : 183.75], [1 : 122.5]]
			for(var dist :var maxDmg in distAOE) {
			//distAOE[0] -> [1 : 245.0]
				var cellsToUse = [];
				CellsToUseTool(weapon,cell,cellsToUse);
				debug("cells to use tool :"+cellsToUse);
				for (var case in cellsToUse) {
					for (var ennemi in casesEnnemis[case]) {
						if (valueWeapons[weapon][dist][ennemi]!==null) {
							if (danger[cell][ennemi]===null) danger[cell][ennemi]= [];
